...
GeoDraw is a language that lets you use basic math equations (lines, circles, parabolas, etc) to construct basic cartoon drawings. This is inspired by a project in high school that was designed to help us learn more about geometry. It was difficult to find a graphing calculator online that made this particular project easy. GeoDraw will be a be a fun educational tool to strengthen students knowledge of geometrical equations.
...
All of the following examples can be run using typing "dotnet run example-X.geo" on the command line from the lang folder, with X representing the example number. Running the code will output a SVG file matching the name of the example in the lang folder. For example, example-1.geo with have an output file example-1.svg. This SVG file can be opened and displayed in a browser. The output of the SVG files are displayed below.
\newpage
...
\begin{tabular}{@{}ll@{}}
  ...
  $<\texttt{Expr}>$ & $::=$ $<$Draw$>$ | <Sequence$>$ \\
	...
	$<$Canvas$>$ & $::=$ $(<$CanvasNum$>,<$CanvasNum$>,<$Color$>$) \\
...
\section{Semantics}
 ...
 We are going to use the following primitive types in generating an expression: x, y, real numbers, operations and equality symbols. These will allow the users to generate the line of almost any equation they wish to draw. Color will also be a primitive type that can be used for setting the background color and for creating lines of various colors.

	\begin{center}
    \begin{tabular}{|p{20mm}|p{35mm}|p{15mm}|p{20mm}|p{55mm}|}
	  \hline
	  ...
	  $X | x$ & varX & Var & N/A & Primitive value used inside of bounds \\
		...
		\hline
		 ...
		 \hline
 		 ‘simple’ $\mid$ ‘funky’ $\mid$ ‘thick’ $\mid$ s & Simple $\mid$ Funky $\mid$ Thick $\mid$ Other of s &
...
\section{Remaining Work:} We would like to implement a custom brush feature, that allows the user to design a brush by entering a list of points. This is the last feature that we plan on implementing.
